...
- [Perseus Vault — Persistent Memory for Claude (formerly Mimir/Mneme)](https://perseus.observer/vault) - Perseus Vault is a local-first, single-binary memory server that gives AI agents persistent, encrypted, bi-temporal memory across sessions via more than 50 MCP tools. *Use case: Store a project decision as an entity so a future session can recall it instead of re-deriving it.*
...
- [Nuvemshop-mcp](https://www.nuvemshop.com.br) - Connects Claude to Nuvemshop (Tiendanube), a Latin American e-commerce platform, to create, manage, analyze, and automate online stores. *Use case: Ask Claude to update product inventory and pull last month's sales report for a Nuvemshop storefront.*
...
- [LogRocket MCP](https://logrocket.com) - A session replay and product analytics platform that lets Claude query LogRocket sessions, metrics, and issues. *Use case: Investigating a user-reported checkout bug by watching the relevant session replay and identifying the root cause.*
...
- [Trip Logistics Assistant](https://triplogistics.app) - Trip logistics booking across six verticals: car rentals, airport transfers, tours and attraction tickets, travel eSIM, luggage storage, and flight compensation. *Use case: Get driver offers for a Vienna airport transfer, pick an eSIM plan for Italy, or check flight-delay compensation eligibility -- each answer is a card with figures and a direct booking link.*
